...
PrismUpdaterApp::PrismUpdaterApp(int& argc, char** argv) : QApplication(argc, argv)
{
    ...
    parser.setApplicationDescription(QObject::tr("An auto-updater for Elysium launcher"));

    parser.addOptions(
        { { { "d", "dir" }, tr("Use a custom path as application root (use '.' for current directory)."), tr("directory") },
          { { "V", "elysiumlauncher-version" },
    ...
    auto version = parser.value("elysiumlauncher-version");
    if (!version.isEmpty()) {
        if (version.contains('-')) {
    ...
    m_updateLogPath = FS::PathCombine(m_dataPath, "logs", "elysiumlauncher_update.log");
...
void PrismUpdaterApp::run()
{
    ...
    if (m_isFlatpak) {
        showFatalErrorMessage(tr("Updating flatpack not supported"), tr("Actions outside of checking if an update is available are not "
                                                                        "supported when running the flatpak version of Elysium Launcher."));
        return;
    }
    if (m_isAppimage) {
        bool result = true;
...
void PrismUpdaterApp::backupAppDir()
{
    ...
    if (file_list.isEmpty()) {
        ...
        if (BuildConfig.BUILD_ARTIFACT.toLower().contains("linux")) {
            file_list.append({ "ElysiumLauncher", "bin", "share", "lib" });
        } else {  // windows by process of elimination
            file_list.append({
                "jars",
                "elysiumlauncher.exe",
                "elysiumlauncher_filelink.exe",
                "elysiumlauncher_updater.exe",
                "qtlogging.ini",
                "imageformats",
                "iconengines",
                "platforms",
